<#
.SYNOPSIS
    OmviHub MSP Client Health Report Generator - Direct Web Bootstrapper [Massgrave Style]
.DESCRIPTION
    A lightweight direct bootstrapper designed to be executed via:
    irm https://toolkit.omvihub.in/health.ps1 | iex   (or irm https://toolkit.omvihub.in/health | iex)
#>
...
if (-not (Test-IsAdmin)) {
    ...
    Write-Host "  [ELEVATION REQUIRED] MSP Client Health Doctor requires Administrator privileges." -ForegroundColor Yellow
    Write-Host "  Requesting UAC Elevation... Please click 'Yes' on the prompt." -ForegroundColor Cyan
    Write-Host "==========================================================================" -ForegroundColor DarkYellow
    
    $CommandLine = "-NoExit -NoProfile -ExecutionPolicy Bypass -Command `"irm https://toolkit.omvihub.in/health.ps1 | iex`""
...
# 3. Download and execute master bootstrapper with -Tool health
Write-Host "[+] Fetching OmviHub MSP Client Health Report Generator engine..." -ForegroundColor Green
...
try {
    ...
    & $BootScript -DownloadUrl $DownloadUrl -InstallDir $InstallDir -Tool "health" -ForceOverwriteAll:$ForceOverwriteAll
} catch {
    Write-Host "[ERROR] Failed to boot Client Health Doctor: $($_.Exception.Message)" -ForegroundColor Red
} finally {
