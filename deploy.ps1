<#
.SYNOPSIS
    OmviHub WPF Software Deployer - Direct Web Bootstrapper [Massgrave Style]
.DESCRIPTION
    A lightweight direct bootstrapper designed to be executed via:
    irm https://toolkit.omvihub.in/deploy.ps1 | iex   (or irm https://toolkit.omvihub.in/deploy | iex)
#>
...
if (-not (Test-IsAdmin)) {
    ...
    Write-Host "  [ELEVATION REQUIRED] WPF Software Deployer requires Administrator privileges." -ForegroundColor Yellow
    Write-Host "  Requesting UAC Elevation... Please click 'Yes' on the prompt." -ForegroundColor Cyan
    Write-Host "==========================================================================" -ForegroundColor DarkYellow
    
    $CommandLine = "-NoExit -NoProfile -ExecutionPolicy Bypass -Command `"irm https://toolkit.omvihub.in/deploy.ps1 | iex`""
...
# 3. Download and execute master bootstrapper with -Tool deploy
Write-Host "[+] Fetching OmviHub WPF Software Deployer engine..." -ForegroundColor Cyan
...
try {
    ...
    & $BootScript -DownloadUrl $DownloadUrl -InstallDir $InstallDir -Tool "deploy" -ForceOverwriteAll:$ForceOverwriteAll
} catch {
    Write-Host "[ERROR] Failed to boot software deployer: $($_.Exception.Message)" -ForegroundColor Red
} finally {
