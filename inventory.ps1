<#
.SYNOPSIS
    OmviHub Hardware & Software Inventory Scanner - Direct Web Bootstrapper [Massgrave Style]
.DESCRIPTION
    A lightweight direct bootstrapper designed to be executed via:
    irm https://toolkit.omvihub.in/inventory.ps1 | iex   (or irm https://toolkit.omvihub.in/inventory | iex)
#>
...
if (-not (Test-IsAdmin)) {
    ...
    Write-Host "  [ELEVATION REQUIRED] Inventory Scanner requires Administrator privileges." -ForegroundColor Yellow
    Write-Host "  Requesting UAC Elevation... Please click 'Yes' on the prompt." -ForegroundColor Cyan
    Write-Host "==========================================================================" -ForegroundColor DarkYellow
    
    $CommandLine = "-NoExit -NoProfile -ExecutionPolicy Bypass -Command `"irm https://toolkit.omvihub.in/inventory.ps1 | iex`""
...
# 3. Download and execute master bootstrapper with -Tool inventory
Write-Host "[+] Fetching OmviHub Hardware & Software Inventory engine..." -ForegroundColor Cyan
...
try {
    ...
    & $BootScript -DownloadUrl $DownloadUrl -InstallDir $InstallDir -Tool "inventory" -ForceOverwriteAll:$ForceOverwriteAll
} catch {
    Write-Host "[ERROR] Failed to boot Inventory Scanner: $($_.Exception.Message)" -ForegroundColor Red
} finally {
